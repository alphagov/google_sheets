...
describe GoogleSheets do
  ...
  let(:key_file) { StringIO.new(key.to_json) }
  let(:invalid_key_file) { StringIO.new( key.tap {|f| f["private_key"] = 'invalid_key' }.to_json ) }

  let(:sheet_id) {'1InL5J5lzvaWGX8ymzotFKGuLUE5xxPg5l7Db-vRhQvY'}
  ...
  it 'gets a spreadsheet' do
    VCR.use_cassette("get_sheet") do
      google_sheets = GoogleSheets.new(key_file: key_file)
      expect(google_sheets.get_document(sheet_id: sheet_id, range: range).values).to eq(data)
    end
  end
  ...
  it 'replaces a spreadsheet' do
    VCR.use_cassette("put_sheet") do
      google_sheets = GoogleSheets.new(key_file: key_file)
      expect(google_sheets.replace_document(sheet_id: sheet_id, data: data).updated_cells).to eq(4)
    end
  end
  ...
  it 'replaces a spreadsheet using a range' do
    VCR.use_cassette("put_sheet_partial") do
      google_sheets = GoogleSheets.new(key_file: key_file)
      expect(google_sheets.replace_document(sheet_id: sheet_id, data: data, range: 'Sheet1!C1:D2').updated_cells).to eq(4)
    end
  end
  ...
  describe 'using an invalid key file' do
    it 'raises an error' do
